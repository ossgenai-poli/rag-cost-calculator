...
| **Context length** | Shown as a derived output: "Context window: 3,410 needed · 8,192 window" | The min = query + sources + prompt + output; the headroom % added; the model's own ceiling | **No silent truncation** (P1-UX-003): `needed > model max` → **infeasible**; `needed ≤ max` but `needed + headroom > max` → recommend the **max** with an explicit *reduced-headroom* warning; else → `needed + headroom`. *(The headroom % is a Phase-1 parameter; until then the wireframe shows needed vs the configured window — see [18-reference-cases.md](18-reference-cases.md) R1.)* |

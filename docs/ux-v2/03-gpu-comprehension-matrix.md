...
| **Context length** | Shown as a derived output: "Context window: 4,500 tok (auto)" | The min = query + sources + prompt + output; the headroom % added; the model's own ceiling | `min(model max, needed + headroom)`; flags overflow as infeasible, never silently truncates |
